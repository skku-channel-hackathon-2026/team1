...
- **앱 안에서 알림과 채팅.** 채널톡 DM은 쓰지 않는다(권한 401로 막혀 팀이 자체 구현으로 전환). 요청·수락·취소·새 메시지는
  이 앱의 알림함에 쌓이고, 목록 화면 우상단 벨에 안 읽은 개수가 뜬다. 서로 수락한 사이에만 상세 화면 아래 1:1 채팅이 열린다.
  알림은 15초, 열려 있는 대화는 5초마다 새로고침한다(웹소켓 없음). 시드 새내기는 데모용으로 짧게 자동 답장한다.
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------- |
| 오프닝          | 아이콘·서비스명 오프닝 화면(최소 1.3초). 그동안 프로필과 추천을 불러오고, 끝나면 페이드 인                               | `tutorial.getProfile`, `tutorial.match`   |
...
notif:<channelId>:<memberId>            → Notification[] (최신순, 최대 50개)
chat:<channelId>:<memberA>|<memberB>    → ChatMessage[] (오래된 순, 최대 200개)
```
...
- 슬롯 = 요일 × 1~10교시. 상태는 수업 인스턴스 / 공강(첫 수업과 마지막 수업 사이 + 첫 수업 직전 1교시 + 마지막 수업 직후 1교시) / 없음.
...
캠퍼스는 자과캠(율전) 하나만 다룬다(팀 결정: 구분이 복잡함). 시드는 소프트웨어학과 중심 12명이다. 예시 시간표와 같은 학과 9명은 겹치는 과목 수가 5·4·3·2·1·0개 순으로 1·1·2·2·2·1명이고, 타 학과 3명은 교양으로만 겹친다. 점수가 89점부터 6점까지 퍼지고, 0개인 사람도 같은 건물이나 공강으로 걸린다. `campus` 필드는 데이터 모델에 남아 있어 나중에 인사캠을 되살릴 수 있다.
1순위 「우주」는 수요일 `공강 1시간 → 2교시 같이 듣기 → 공강 1시간 → 4교시 같이 듣기 → 공강 1시간 → 6교시 같이 듣기 → 공강 1시간`이 나온다.
