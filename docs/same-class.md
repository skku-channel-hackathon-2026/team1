...
- **점수를 숫자로 보여준다.** PRD는 % 표기를 금지했지만 팀 결정으로 카드에 `23.9점`, 상세 화면에 항목별 내역(같은 강의실 / 같은 건물 /
  공강 / 체인 / 동행 / 점심)을 막대로 보여준다. 내역은 서버 `MatchResult.breakdown`에서 온다.
...
| `tutorial.requestMatch`  | `{ targetId }` | `{ targetId, matchState, revealedInstances? }` |
...
- 매칭 전: 닉네임, 근접 등급 배지, 이유 3줄, 겹치는 칸 위치(과목명은 내가 듣는 수업이라 표시). 상대 단독 수업·강의실 호수는 서버가 보내지 않는다.
- 상호 수락: `revealedInstances`로 상대 시간표 전체가 열린다.
...
- 요약 문장의 「끝나고」는 체인 판정을 그대로 따른다. 같은 수업 뒤에 내 수업이 하나 끼어 있으면 체인이 아니고 문장에도 붙지 않는다.
...
1순위 「하늘」은 수요일 `2교시 같이 듣고 → 끝나고 점심 함께 → 4교시 같이 듣고 → 끝나고 공강 1시간 → 6교시 같이 듣고`가 나온다.
