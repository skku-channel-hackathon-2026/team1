...
- **다이렉트 메시지 알림.** 요청과 성사 시 상대 매니저에게 1:1 메시지를 보낸다. 채널톡에는 봇 DM이 없어서
  `findOrCreateDirectChat`으로 두 사람의 DM 방을 찾거나 만들고, `writeDirectChatMessageAsManager`로 **요청한 사람 명의**로 보낸다.
  시드 새내기는 매니저 계정이 없어 보내지 않는다. 전송 실패는 요청 처리에 영향을 주지 않고 `notified: false`로만 돌아온다.
  전송은 캐시된 채널 토큰 → 새로 발급한 채널 토큰 → 앱 토큰 순으로 시도하고, 실패 이유를 `notifyError`로 화면에 보여준다.
  서버가 실패하면 WAM이 매니저 본인 세션으로 같은 두 함수를 한 번 더 시도한다.
  그래도 401이면 앱 권한 문제이므로 운영진에게 두 함수 권한 추가를 요청해야 한다.
...
- 보너스 = 체인(같은 수업 직후 둘 다 공강) `3.0` · 점심 겹침 요일 `2.0`. 동행 이동 보너스는 제거했다.
...
시드는 캠퍼스마다 29명(인사캠 경영학과 중심, 자과캠 소프트웨어학과 중심)이고 구조가 같다. 예시 시간표와 같은 학과 23명 중 인스턴스 3개 이상 겹침 2명, 1~2개 8명, 0개 13명이다. 예시 시간표도 캠퍼스별로 따로 있어 캠퍼스를 바꾸면 수업·교수·건물이 통째로 바뀐다.
인사캠 1순위 「하늘」(자과캠은 「우주」)은 53점, 수요일 `2교시 같이 듣기 → 공강 1시간 → 4교시 같이 듣기 → 공강 1시간 → 6교시 같이 듣기`가 나온다.
